(defun day-1-1 ()
  (let ((list (with-open-file (f "../input/day-1-1.input")
		(do* ((n (read-line f))
		      (len (length n))
		      (i 0 (1+ i))
		      (res (list)))
...
(defun day-2-1 ()
  (labels ((setup () (with-open-file (f "../input/day-2-1.input")
...
(defun day-4 ()
  (let ((pass (with-open-file (f "../input/day-4.input")
		(do ((l (read-line f nil nil)
			(read-line f nil nil))
		     (out (list)))
		    ((null l) out)
...
(defun day-6 ()
  (let ((blocks (with-open-file (f "../input/day-6.input")
		  (let ((l (read-line f)))
		    (loop for i = 0 then (1+ j)
		       as j = (position #\Tab l
					:start i)
...
(defun day-12 ()
  (let ((connections (make-hash-table)))
    (with-open-file (f "../input/day-12.input")
      (do ((l (read-line f nil nil)
	      (read-line f nil nil)))
	  ((null l))
...
(defun day-9 ()
  (labels ((process-group (string &optional (position 0) (score 1))
    ...
    (let ((stream  ;"{{{}}}"))
	   (with-open-file (f "../input/day-9.input")
	     (read-line f))))
...
(defun day-5 ()
  (let ((instructions (with-open-file (f "../input/day-5.input")
			(do ((l (read-line f nil nil)
				(read-line f nil nil))
			     (out (list)))
			    ((null l) (reverse out))
...
(defun day-7 ()
  (let ((tree    (make-hash-table :test #'equalp))
	...
	(last-name nil))
    (with-open-file (f "../input/day-7.input")
      (do ((l (read-line f nil nil)
	      (read-line f nil nil))
	   (name nil))
...
(defun day-8 ()
  (let ((instructions (with-open-file (f "../input/day-8.input")
			(do ((l (read-line f nil nil)
				(read-line f nil nil))
			     (out (list)))
			    ((null l) (reverse out))
...
(defun day-23 ()
  (let ((instructions (with-open-file (f "../input/day-23.input")
			(do ((l (read-line f nil nil)
				(read-line f nil nil))
			     (out (list)))
			    ((null l) (reverse out))
